/**
 * Script de teste: Testa a extração de dados de uma única página de mangá
 *
 * Uso: npx tsx src/test-single.ts [url]
 */
...
async function main() {
  console.log('\n========================================');
  console.log('   TESTE DE EXTRAÇÃO - LERMANGAS.ME');
  console.log('========================================\n');

  // URL de teste (pode ser passada como argumento)
  const testUrl = process.argv[2] || 'https://lermangas.me/manga/a-dama-bebe-controla-o-mundo-com-dinheiro/';

  try {
    const data = await extractMangaData(testUrl);

    console.log('\n--- DADOS EXTRAÍDOS ---\n');
    console.log(`URL: ${data.url}`);
    console.log(`Título: ${data.title}`);
    console.log(`Nomes Alternativos: ${data.alternativeNames.length > 0 ? data.alternativeNames.join(', ') : '(nenhum)'}`);
    console.log(`Imagem: ${data.imageUrl || '(não encontrada)'}`);
    console.log(`Synopsis: ${data.synopsis ? data.synopsis.substring(0, 200) + '...' : '(não encontrada)'}`);
    console.log(`Gêneros: ${data.genres.length > 0 ? data.genres.join(', ') : '(nenhum)'}`);
    console.log(`Autor: ${data.author || '(não encontrado)'}`);
    console.log(`Artista: ${data.artist || '(não encontrado)'}`);
    console.log(`Status: ${data.status || '(não encontrado)'}`);
    console.log(`Total Capítulos: ${data.totalChapters}`);
    console.log(`Rating: ${data.rating !== null ? data.rating : '(não encontrado)'}`);
